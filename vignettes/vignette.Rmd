...
The example dataset is downloaded from 
http://imlspenticton.uzh.ch/robinson_lab/conquer/data-mae/GSE48968-GPL13112.rds.
After performing quality control procedure of genes and cells 
(as outlined in the paper),
we end up with 1059 cells. The genes selected for this analysis is 
the top variant 2000 genes that are ranked by its variance in raw counts. 
```{r}
...
$\beta$ is corresponding coefficient matrix and is named as "beta" in the model object
4. $\gamma$ if there is gene-level covariates (e.g batch effect), 
$\gamma$ is corresponding coefficient matrix and is named as "gamma" in the model object
