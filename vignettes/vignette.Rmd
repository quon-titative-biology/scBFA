...
The example dataset is generated from our scRNA-seq simulator. 
As outlined in our paper, the true parameters are learned from a ZINB-WaVE
model learned from fitting a real dataset 
https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE89232
After performing quality control procedure of genes and cells
(as outlined in the paper), we filtered out genes that are undetected in more 
than 99% of cells and remove cells that has in more than 99% genes undetected.
we end up with 950 cells and 2000 genes
```{r}
...
```
## Simulate a raw count matrix and corresponding cell labels

```{r,include=TRUE,message=FALSE}
set.seed(1)

scData = scNoiseSim(zinb = zinb,celltype = celltype,disper =5,var_dropout =0.1,
            var_count = 1,
            delta = -1)

exprdata = scData$exprdata;

celltype = scData$celltype;

print(paste("dimensions of the expression profile:",
            nrow(exprdata),"genes and",ncol(exprdata),"cells"))
...
We choose 10 as number of latent dimensions and project the gene detection 
...
tsne_bfa = Rtsne(scale(bfa_model$ZZ),
                initial_dims =10,perplexity =30,Steps = 5000)
...
```{r,fig.width=8, fig.height=6}
set.seed(5)
tsne_bpca = Rtsne(scale(bpca$x[,c(1:10)]),
                initial_dims =10, perplexity = 30,Steps = 5000)
